...
if( ! function_exists( 'woodmart_search_form' ) ) {
	function woodmart_search_form( $args = array() ) {
		...
		switch ( $post_type ) {
			case 'product':
				$placeholder = esc_attr_x( 'Procure seu produto', 'submit button', 'woodmart' );
...
if ( ! function_exists( 'woodmart_ajax_suggestions' ) ) {
	function woodmart_ajax_suggestions() {
		...
		if ( $results->have_posts() ) {
			...
			$suggestions[] = array(
				'value' => ( $post_type == 'product' ) ? esc_html__( 'Nenhum produto encontrado', 'woodmart' ) : esc_html__( 'No posts found', 'woodmart' ),
				'no_found' => true,
				'permalink' => ''
			);
		}
